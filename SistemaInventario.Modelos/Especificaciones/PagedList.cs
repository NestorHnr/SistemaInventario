    ...
    public class PagedList<T>:List<T>
    {
        ...
        public PagedList(List<T> items, int count, int pageNumber, int pageSize)
        {
            MetaData = new MetaData()
            {
                TotalCount = count,
                PageSize = pageNumber,
                TotalPages =  (int)Math.Ceiling(count / (double)pageSize)  //por Ejemplo un 1.5 lo transforma en 2
            };
            AddRange(items); //Agrega los elementos de la colecccion al final de la lista
        }
